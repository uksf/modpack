	...
	case "onLoad": {
		...
		_params spawn {
			...
			while {_display == uinamespace getvariable ["RscMissionStatus_display",_display]} do {
				...
				// Exclusive Vanguard MP functionality: Hack to force show the control
				if (missionNamespace getVariable ["BIS_ak_zoneTimer",-10] != -10) then {_scoreTotal = 1};
				...
				// --- handle Apex COOP differently
				if (_isApexCOOPOverride) then {_showTickets = false};
				
				//--- Show values. Tickets always, score based on difficulty and only if at least one side has more than 0 points
				if (_showTickets || (!_showTickets && (difficultyOption "scoreTable" > 0) && _scoreTotal > 0)) then {
					{
						_side = _x;
				...
				if ((ismultiplayer || cheatsenabled) && !((player call bis_fnc_objectSide) in _ticketSides) && !isnull group player) then {
					...
					{
						// --- handle Apex COOP differently
						_color = if (_isApexCOOPOverride) then
						{	
							call 
							{
								if (_tickets != _oldTickets) exitWith
								{
									_tmp = _tickets; _tickets = _oldTickets; _oldTickets = _tmp; // swap
									[0.0,0.0,0.0,0.6]; // flash black on change
								};
								if (_tickets == 0) exitWith {[1.0,0.0,0.0,0.9]}; // red
								if (_tickets <= 5) exitWith {[1.0,0.5,0.0,0.75]}; // orange
								[0.0,1.0,0.0,0.5] // green
							};
						} 
						else
						{
							if (_tickets == 0) exitWith {_colorError};
							if (_tickets <= 5) exitWith {_colorWarning};
							[0,0,0,0.35]
						};
