...
[{
    ...
    if (_debug) then {
		...
		_markers = [];
        {_markerstr = createMarker [format ["%1_%2", _forEachIndex], leader _x]; _markerstr setMarkerShape "ICON"; _markerstr setMarkerType "hd_dot"; _markers pushBack _markerstr;} forEach _currentGroups;
        _markerName setMarkerAlpha 1;
		missionNamespace setVariable [format ["%1_markers", _markerName], _markers, true];
    };
}, 10, [_markerName, _maxAmount, _factionName, _debug]] call CBA_fnc_addPerFrameHandler;
