...
/*
    ...
    Description:
        Gets romeo actions for container
    ...
    Example:
        [_target] call uksf_resupply_fnc_getRomeoActions
*/
params ["_target"];

private _actions = [];

private _action = [QGVAR(reviewTypes), "Review Romeo Crates", "", {
    private _descriptions = GVAR(romeoClasses) apply {[_x#1, _x#2] joinString " - "};

    private _text = format ["<t color='#fec400'>Available Romeo Crates:</t><t><br/><br/>%1</t>", _descriptions joinString "<br/>"];
    private _structuredText = composeText [lineBreak, parseText _text];
    [_structuredText, 2 + (0.5 * count _descriptions), ACE_player, 15] call ace_common_fnc_displayTextStructured;
