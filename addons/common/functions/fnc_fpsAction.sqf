...
if (local player && !(player getVariable [QGVAR(fpsActionAdded), false])) then {
    _action = [QGVAR(fpsAction), QUOTE(FPS), "\A3\ui_f\data\gui\rsc\rscdisplayoptionsvideo\icon_performance.paa", {call FUNC(fpsToggle)}, {ADMIN_OR_HOST}] call ace_interact_menu_fnc_createAction;
