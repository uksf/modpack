...
// TODO: Generalise this to be used in all aircraft. Will need better calculation to determine if a missile goes for counter measures (also consider counter measure type and missile seeker type)

private _cmMode = _aircraft getVariable [QGVAR(cmMode), 0];
if (_cmMode > 0 && {_missileType isKindOf "MissileBase"}) then {
    private _missile = nearestObject [_hostile, _missileType];

    if (_missile getVariable [QGVAR(isGuiding), false]) exitWith {};
    _missile setVariable [QGVAR(isGuiding), true, true];
