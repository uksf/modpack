...
if (local _heli && {alive _heli} && {player isEqualTo (driver _heli) || {player isEqualTo (gunner _heli)}} && {isEngineOn _heli} && {_missileType isKindOf "MissileBase"} && {(_heli getVariable [QGVAR(jammerMode), 0]) > 0}) then {
    ...
    [format [QUOTE(GVAR(%1oclock)), _oclock], 2.1, format [QUOTE(GVAR(%1)), _missileAlt], 0.6] call FUNC(audio);
    ...
    if (player isEqualTo (driver _heli) && {(_heli getVariable [QGVAR(jammerMode), 0]) isEqualTo 2}) then {
        [{
            ...
            if (isNull _missile || {!alive _heli} || {_iteration >= 21} || {(_missile distance _heli) > (10000)}) exitWith {
    ...
    [{
        ...
        private _flareList = _heli nearEntities ["CMflareAmmo", FLARESEARCHRADIUS];
        ...
        if (_missileAlt != "high" && {(floor random 100) >= TRACKCHANCE} && {(count _flareList) > 0}) then {
            ...
            [{
                ...
                if (_distanceToTarget < DETONATIONDISTANCE || {_distanceToTarget > _prevDistanceToTarget}) then {
    ...
    }, [_heli, _missile], 0.3] call CBA_fnc_waitAndExecute;    
};
