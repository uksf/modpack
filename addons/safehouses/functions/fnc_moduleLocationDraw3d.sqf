...
/*
    ...
    Description:
        Draws bounding boxes for safehouse location
...
private _modules = (get3DENSelected "logic") select {typeOf _x == QGVAR(moduleLocation)};
if (_modules isEqualTo []) exitWith {};

private _logic = _modules#0;
private _building = _logic getVariable [QGVAR(building), objNull];
if (isNull _building) exitWith {};

private _boundingPoints = _building getVariable [QGVAR(boundingPoints), []];
if (_boundingPoints isEqualTo []) then {
    ...
    _boundingPoints = [
        _building modelToWorld _top, _building modelToWorld [_bottom#0, _top#1, _top#2], _building modelToWorld [_bottom#0, _bottom#1, _top#2], _building modelToWorld [_top#0, _bottom#1, _top#2],
        _building modelToWorld _bottom, _building modelToWorld [_top#0, _bottom#1, _bottom#2], _building modelToWorld [_top#0, _top#1, _bottom#2], _building modelToWorld [_bottom#0, _top#1, _bottom#2]
    ];

    _building setVariable [QGVAR(boundingPoints), _boundingPoints];
};
