...
private _size = _logic getVariable ["objectArea", [0,0]];
private _sizeXandY = [_size#0, _size#1];
...
[{
    params ["_args", "_idPFH"];
    _args params ["_logic", "_condition", "_size", "_lockAfterCapture", "_requiredDominance", "_timeToCapture", "_markers", "_name"];
    ...
    // one up one down
    // TODO: Need to make this DRY somehow
    if (_currentDominantSide == 0) then {
        ...
        [_entities, _currentOpforScore, _timeToCapture] call FUNC(getSectorCaptureProgress);
    };
    ...
    if (_currentDominantSide == 1) then {
        ...
        // show a hint percentage
        [_entities, _currentBluforScore, _timeToCapture] call FUNC(getSectorCaptureProgress);
    };

    // fire ownerChanged event on score/time to capture met
    if (_currentOpforScore >= _timeToCapture && _currentOwner != 0) then {
        [QGVAR(sectorOwnerChanged), [0, _currentOwner, _logic, _lockAfterCapture, _name]] call CBA_fnc_serverEvent;
    };

    if (_currentBluforScore >= _timeToCapture && _currentOwner != 1) then {
        [QGVAR(sectorOwnerChanged), [1, _currentOwner, _logic, _lockAfterCapture, _name]] call CBA_fnc_serverEvent;
    };

}, 1, [_logic, _condition, _size, _lockAfterCapture, _requiredDominance, _timeToCapture, _markers, _name]] call cba_fnc_addPerFrameHandler;
