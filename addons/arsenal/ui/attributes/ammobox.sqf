...
switch _mode do {
	case "onLoad": {
		...
		if ({count (_x select 0) > 0} count _cargo == 0) then {
			AmmoBox_type = 1;
			_virtualCargo = [
				_entity call uksf_arsenal_fnc_getVirtualWeaponCargo,
				_entity call uksf_arsenal_fnc_getVirtualMagazineCargo,
				_entity call uksf_arsenal_fnc_getVirtualItemCargo,
				_entity call uksf_arsenal_fnc_getVirtualBackpackCargo
			];
			{
