 ...
 *
 * Edited to remove ascension shit, eagle, loaded modules (aramzeuscache overwrites it), and SP curator dumb message
 *
 * Arguments:
 * 0: Logic <OBJECT>
 * 1: Units <ARRAY>
...
if (_activated) then {
    ...
    if (_ownerVar == "" && {!isMultiplayer}) then {
        // ["Curator owner not defined, player used instead in singleplayer."] call BIS_fnc_error; // FUCKING WHY! Don't spam shitty usless messages if you're just going to set the damn variable anyway
