...
if (_activated) then {
    ...
    if (isserver) then {
        ...
        [_logic,_ownerVar,_ownerUID,_adminVar] spawn {
            ...
            while {true} do {
                ...
                private _msgCode = {
                    ...
                    if ((_logic getVariable ["showNotification",true]) && ace_zeus_zeusAscension) then {
                        {
                            if (isplayer _x) then {
                ...
                if !(ace_common_settingsInitFinished) then {
                    ace_common_runAtSettingsInitialized pushBack [_msgCode, [_logic,_player]];
                } else {
        ...
        if !(ace_common_settingsInitFinished) then {
            ace_common_runAtSettingsInitialized pushBack [_birdCode, [_logic]];
        } else {
