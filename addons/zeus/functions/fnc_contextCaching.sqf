...
/*
    ...
    Example:
        [_selectedObjects, _selectedGroups] call uksf_zeus_fnc_contextCaching
*/
...
if (_selectedObjects isNotEqualTo []) exitWith {
    ...
    private _state = [_selectedObjects, {!(_x getVariable [QEGVAR(caching,excluded), false])}] call EFUNC(common,arrayAny); // At least 1 included = true
    {
        [[QEGVAR(caching,enableCache), QEGVAR(caching,disableCache)] select _state, [_x]] call CBA_fnc_serverEvent; // If included, exclude
    } forEach _selectedGroups;
};
