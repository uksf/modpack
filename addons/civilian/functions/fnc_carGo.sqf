...
/*
    ...
    Description:
        Searches for civilian vehicles in front of unit, and commands them to go
...
if (time < (GVAR(lastGesture) + GESTURE_COOLDOWN)) exitWith {DEBUG("Go called within cooldown, exiting")};
...
private _index = _entities findIf {
    ...
    if (isNull _driver) then {
        _driver = _vehicle getVariable [QGVAR(stop_statemachine_driver), objNull];
    };
