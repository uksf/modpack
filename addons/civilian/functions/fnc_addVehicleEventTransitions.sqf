...
[GVAR(vehicle_statemachine), QGVAR(vehicle_state_stopping), QGVAR(vehicle_state_move), [QGVAR(moveCommand)], {true}, {
    ...
    _vehicle setVariable [QGVAR(vehicle_statemachine_annoyed), _annoyed + RANDOM_ANNOYED_STOPPING_TO_MOVE, true];
    TRACE_3("Adding to annoyed",_vehicle,_annoyed,_vehicle getVariable [ARR_2(QGVAR(vehicle_statemachine_annoyed),0)]);
...
[GVAR(vehicle_statemachine), QGVAR(vehicle_state_stopped), QGVAR(vehicle_state_move), [QGVAR(moveCommand)], {true}, {
    ...
    private _driver = driver _vehicle;
    private _annoyed = _vehicle getVariable [QGVAR(vehicle_statemachine_annoyed), 0];
    _vehicle setVariable [QGVAR(vehicle_statemachine_annoyed), _annoyed + RANDOM_ANNOYED_STOPPED_TO_MOVE, true];
    TRACE_3("Adding to annoyed",_vehicle,_annoyed,_vehicle getVariable [ARR_2(QGVAR(vehicle_statemachine_annoyed),0)]);
...
[GVAR(vehicle_statemachine), QGVAR(vehicle_state_move), QGVAR(vehicle_state_stopping), [QGVAR(stopCommand)], {true}, {
    ...
    private _driver = driver _vehicle;
    private _annoyed = _vehicle getVariable [QGVAR(vehicle_statemachine_annoyed), 0];
    _vehicle setVariable [QGVAR(vehicle_statemachine_annoyed), _annoyed + RANDOM_ANNOYED_MOVE_TO_STOPPING, true];
