...
/*
    ...
    Description:
        Searches for civilian units in front of unit, and stops closest one if found
...
if (_index != -1) then {
    ...
    if ((acos ((eyeDirection _driver) vectorCos ((eyePos _driver) vectorFromTo (eyePos _unit)))) < (VEHICLE_VISION_ARC / 2)) then {
